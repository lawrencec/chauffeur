...
Driver._checkResultAgainstExpectation = function _checkResultAgainstExpectation(expectation, expectationKeys, msg, result) {
  var errorMsg;
  if (expectationKeys) {
    expectationKeys.forEach(function(item) {
      if (result[item] !== expectation[item]) {
...
Driver._getExpectationCallback = function _getExpectationCallback(expectation, msg, expectationKeys) {
  ...
  return function(err, result) {
    ...
    return Driver._checkResultAgainstExpectation(expectation, expectationKeys, msg, result);
  };
};
Driver.prototype = {
  init: function init() {
    this._remoteDriver.init();
  },

...
WebDriverJS.prototype = {
  ...
  elements: function elements(selector, expectation, coerceFn, cb) {
    var callback = Driver._getExpectationCallback(
        expectation,
        ERROR_MSGS.resolve(ERROR_MSGS.INCORRECT_NUMBER_FOUND),
        null,
        coerceFn,
        cb
    ).bind(this);
  ...
  getWindow: function getWindow() {
    return new webdriver.WebDriver.Window(this._remoteDriver);
  },
  ...
  getSize: function getSize(callback) {
    ...
    window.getSize()
        .then(function(result) {
          callback(null, {width: result.width, height: result.height});
        });
  },

  takeScreenshot: function takeScreenshot(imagePath) {
    this._remoteDriver.takeScreenshot()
        .then(function(base64_png) {
          fs.writeFile(imagePath, base64_png, 'base64', function(err) {
            if (err) {
              ERROR_MSGS.resolve(
                Driver._errorOut(ERROR_MSGS.SCREENSHOT_UNSUCCESSFUL, err)
              );
            }
          });
        })
